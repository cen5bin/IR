...
public class KMeans {
	...
	private static void initSeeds(ArrayList<VSMDoc> docList, int K)					
	{
		...
		for (int i = 0; i < K; i++)
		{
			...
			pos += offset;
			clusterCenters.put(i, doc.getDocVector()); //key是簇标号，value是向量
		}
	}
	...
	private static void partitionToCluster(ArrayList<VSMDoc> docList)								
	{
		for (int i = 0; i < docList.size(); i++)
		{
			...
			cluserMap.put(docList.get(i).getDocId(), clusterToBelong); //为文档向量分配到最近的簇
		}
	}
	...
	private static void reCalculateCenter(ArrayList<VSMDoc> docList, int K)
	{
		...
		for (int k = 0; k < K; k++) //为每一个簇计算新的中心向量
		{
